...
* [☁️ Интеграция с хранилищем](#-интеграция-с-хранилищем)
* [⚠️ Анализ устойчивости](#-анализ-устойчивости)
...
| ---------------- | ------------------------------------- | ---------------------------- | --------------------------- |
| load\_data.py    | загрузка датасета                     | исходный csv файл            | csv с названиями столбцов   |
| clean\_data.py   | очистка и предобработка               | csv с названиями столбцов    | очищенный csv               |
| train\_model.py  | обучение модели с нормализацией       | очищенный csv                | файл обученной модели (.pkl)|
| calc\_metrics.py | расчёт метрик на тестовой выборке     | очищенный csv, модель (.pkl) | json с метриками            |
...
* **Зависимости:** загрузка → очистка → обучение → расчёт метрик
* **Структура DAG:** ```load_data >> clean_data >> train_model >> calc_metrics```

### Описание операторов

* `PythonOperator` с функцией `run_script(script_name)` запускает каждый скрипт из `.py` скриптов через `subprocess`.
* Логи пишутся в `logs/`.
...
4. **Включите DAG `breast_cancer_pipeline` и запустите его вручную через интерфейс**
    ...
    *или с помощью CLI (необходимо указать актуальную дату):*
    ```bash
    airflow tasks test breast_cancer_pipeline load_data <YYYY-MM-DD>
    ```

6. **Результаты сохраняются в папку `results/`:**
...
| `train_model.py`  | Ошибка при обучении модели, ошибка sklearn     | Обернуто в `try/except`, логгируется |
...
* Использование облачных хранилищ (Google Drive API, Dropbox API, Amazon S3-compatible) вместо локального диска для хранения результатов.
* Добавление тестов данных.
...
📌 Вывод: Модель показала высокие показатели точности и чувствительности, что особенно важно в медицинских задачах. Уровень recall достаточно высокий, это означает, что большинство положительных случаев выявлено, а высокий precision — что среди выявленных случаев почти нет ложноположительных. Для развития модели можно увеличить показатель recall.
---

**Автор:** © 2025 NatalieGo

**Проект:** ETL для классификации рака груди с использованием Airflow

🔗 [Репозиторий проекта](https://github.com/NatalieGo/exam-engineering)
