...
* `clean_data.py` — очистка и предобработка данных

* `train_model.py` — обучение модели логистической регрессии с нормализацией

* `calc_metrics.py` — расчет метрик качества модели и сохранение результатов
